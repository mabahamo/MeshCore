...
#ifdef ESP32
  // Forward packet to HTTP endpoint
...
#endif
{
  ...
  _prefs.advert_interval = 0;        // DISABLE local advertisements for forwarder
  _prefs.flood_advert_interval = 0;  // DISABLE flood advertisements for forwarder
  _prefs.flood_max = 64;
  _prefs.interference_threshold = 0; // disabled
  _prefs.disable_fwd = 1;            // DISABLE packet forwarding/repeating for forwarder
...
#ifdef ESP32
  // Periodic WiFi check (every 10 seconds)
...
#ifdef ESP32
// WiFi and HTTP forwarding methods

bool MyMesh::initWiFi(const char* ap_name) {
  Serial.println("Initializing WiFi...");

  // Load all preferences
  _prefs_storage.begin("forwarder", false);
  loadForwardingHost();
  loadLoRaParams();
  ...
  // Convert LoRa parameters to strings for WiFiManager
  char freq_str[16], bw_str[16], sf_str[8], cr_str[8], power_str[8];
  snprintf(freq_str, sizeof(freq_str), "%.3f", _lora_freq);
  snprintf(bw_str, sizeof(bw_str), "%.1f", _lora_bw);
  snprintf(sf_str, sizeof(sf_str), "%d", _lora_sf);
  snprintf(cr_str, sizeof(cr_str), "%d", _lora_cr);
  snprintf(power_str, sizeof(power_str), "%d", _lora_tx_power);

  // Custom parameters
  WiFiManagerParameter custom_host("host", "Forwarding Host", _forward_host, 127);
  WiFiManagerParameter custom_freq("freq", "LoRa Frequency (MHz)", freq_str, 15);
  WiFiManagerParameter custom_bw("bw", "Bandwidth (kHz)", bw_str, 15);
  WiFiManagerParameter custom_sf("sf", "Spreading Factor (7-12)", sf_str, 7);
  WiFiManagerParameter custom_cr("cr", "Coding Rate (5-8)", cr_str, 7);
  WiFiManagerParameter custom_power("power", "TX Power (dBm)", power_str, 7);

  wm.addParameter(&custom_host);
  wm.addParameter(&custom_freq);
  wm.addParameter(&custom_bw);
  wm.addParameter(&custom_sf);
  wm.addParameter(&custom_cr);
  wm.addParameter(&custom_power);

  // Set callback to save custom parameters
  wm.setSaveParamsCallback([this, &custom_host, &custom_freq, &custom_bw, &custom_sf, &custom_cr, &custom_power]() {
    saveForwardingHost(custom_host.getValue());

    // Parse and save LoRa parameters
    _lora_freq = atof(custom_freq.getValue());
    _lora_bw = atof(custom_bw.getValue());
    _lora_sf = atoi(custom_sf.getValue());
    _lora_cr = atoi(custom_cr.getValue());
    _lora_tx_power = atoi(custom_power.getValue());

    // Validate and constrain values
    if (_lora_freq < 100.0 || _lora_freq > 1000.0) _lora_freq = 915.800;
    if (_lora_bw < 7.8 || _lora_bw > 500.0) _lora_bw = 250.0;
    if (_lora_sf < 7 || _lora_sf > 12) _lora_sf = 12;
    if (_lora_cr < 5 || _lora_cr > 8) _lora_cr = 8;
    if (_lora_tx_power < 2 || _lora_tx_power > 30) _lora_tx_power = 22;
...
void MyMesh::loadForwardingHost() {
  String host = _prefs_storage.getString("forward_host", "");
  if (host.length() > 0) {
...
void MyMesh::saveForwardingHost(const char* host) {
  _prefs_storage.putString("forward_host", host);
  strncpy(_forward_host, host, sizeof(_forward_host) - 1);
  _forward_host[sizeof(_forward_host) - 1] = 0;
  Serial.print("Saved forwarding host: ");
  Serial.println(_forward_host);
}

void MyMesh::loadLoRaParams() {
  _lora_freq = _prefs_storage.getFloat("lora_freq", 915.800);
  _lora_bw = _prefs_storage.getFloat("lora_bw", 250.0);
  _lora_sf = _prefs_storage.getUChar("lora_sf", 12);
  _lora_cr = _prefs_storage.getUChar("lora_cr", 8);
  _lora_tx_power = _prefs_storage.getUChar("lora_power", 22);

  Serial.println("Loaded LoRa parameters from storage");
}

void MyMesh::saveLoRaParams() {
  _prefs_storage.putFloat("lora_freq", _lora_freq);
  _prefs_storage.putFloat("lora_bw", _lora_bw);
  _prefs_storage.putUChar("lora_sf", _lora_sf);
  _prefs_storage.putUChar("lora_cr", _lora_cr);
  _prefs_storage.putUChar("lora_power", _lora_tx_power);

  Serial.println("Saved LoRa parameters to storage");
}

void MyMesh::applyLoRaParams() {
  Serial.println("Applying LoRa parameters to radio...");
  radio_set_params(_lora_freq, _lora_bw, _lora_sf, _lora_cr);
  radio_set_tx_power(_lora_tx_power);

  // Also update _prefs so display shows correct values
  _prefs.freq = _lora_freq;
  _prefs.bw = _lora_bw;
  _prefs.sf = _lora_sf;
  _prefs.cr = _lora_cr;
  _prefs.tx_power_dbm = _lora_tx_power;

  Serial.println("LoRa parameters applied successfully");
}

bool MyMesh::ensureWiFiConnected() {
...
void MyMesh::forwardPacket(mesh::Packet* pkt, const uint8_t* raw_packet, int len, float snr, float rssi) {
  ...
  // Build URL: https://{DOMAIN}/incoming
  snprintf(url, sizeof(url), "https://%s/incoming", _forward_host);
  ...
  json += ",\"timestamp\":";
  json += String(getRTCClock()->getCurrentTime());
  json += "}";
  ...
  if (httpCode > 0) {
    Serial.printf("Forwarded packet, HTTP code: %d\n", httpCode);
