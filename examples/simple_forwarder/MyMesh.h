...
#define FIRMWARE_ROLE "forwarder"
...
class MyMesh : public mesh::Mesh, public CommonCLICallbacks {
...
#ifdef ESP32
  // WiFi and HTTP forwarding
  Preferences _prefs_storage;
  ...
  // LoRa parameters (stored separately from NodePrefs)
  float _lora_freq;
  float _lora_bw;
  uint8_t _lora_sf;
  uint8_t _lora_cr;
...
#ifdef ESP32
  // WiFi and forwarding management
  bool initWiFi(const char* ap_name);
  const char* getForwardingHost() const { return _forward_host; }
  uint32_t getPacketsForwarded() const { return _packets_forwarded; }
  uint32_t getForwardFailures() const { return _forward_failures; }
